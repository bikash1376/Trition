...
**Current phase: sidebar now reflects workspace/page create+rename in real time (a shared `SidebarRefreshProvider`), public workspaces get a distinct globe icon + a copyable Trello share link, sidebar pages gained an edit (rename) icon next to delete, workspaces can be permanently deleted (type-to-confirm, Trello's real `DELETE /1/boards/{id}`), the small uppercase section labels site-wide switched to Newsreader (serif, title case, not uppercase), and full SEO metadata (OG image, favicon, robots.txt, sitemap.xml) was added. See Phase 34.**
