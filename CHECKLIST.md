...
**Current phase: three-way theme system (Light/Dark/Exp), a real pink design language for Light + Exp with dashed-outline "create"-style buttons, card-detail-sheet now shows and edits custom columns and lets you delete attachments, bookmark blocks show a real favicon + meta description, two real-time bugs fixed (Board Settings sheet not scrolling, sidebar page list not updating after delete). See Phase 33. Not yet committed — awaiting the go-ahead.**
