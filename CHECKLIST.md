...
**Current phase: swapped the post-auth "setting up" video for a shadcn/Base UI Progress bar (min. 3.5s display), and a round of sidebar + dialog + card-sheet layout fixes — trigger alignment, board-type icons (org vs. private/public), radio-label text alignment, a shimmer hover on the "Trition" wordmark, smaller/bigger "+" affordances, and an icon-only empty-pages state. See Phase 27. Not yet committed — awaiting the go-ahead.**
